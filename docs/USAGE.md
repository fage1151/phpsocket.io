...
```php
// 全局中间件 - 只在连接时执行，作用于所有命名空间
$io->use(function ($socket, $next) {
    $sid = $socket->id;
    $namespace = $socket->namespace;
    
    echo "[Global Middleware] Connection request: SID={$sid}, Namespace={$namespace}\n";
    
    // 验证 auth
    $auth = $socket->handshake['auth'] ?? null;
    if ($auth) {
        echo "[Global Middleware] Auth: " . json_encode($auth) . "\n";
        
        // 如果需要，可以通过 next($error) 拒绝连接
        // $error = new \Exception('Invalid auth');
        // $error->data = ['code' => 401];
        // $next($error);
        // return;
    }
...
```php
// 命名空间级中间件 - 只在连接时执行，只作用于 /chat 命名空间
$io->of('/chat')->use(function ($socket, $next) {
    echo "[Namespace Middleware] /chat namespace connection\n";
    $next();
});
```
...
| `recovered` | bool | 连接是否已成功恢复（重连时恢复状态） |
...
$io->on('connection', function ($socket) {
    if ($socket->recovered) {
        ...
        // 此时 $socket->data 已自动恢复
        // 房间信息也已自动恢复
        
        // 可以检查 $socket->data 中是否有需要续传的数据
        if (isset($socket->data['pendingMessages'])) {
            foreach ($socket->data['pendingMessages'] as $msg) {
                $socket->emit('message', $msg);
            }
            unset($socket->data['pendingMessages']);
        }
...
- **v1.5.0**：完整实现 Socket.IO v4 连接恢复机制，添加 `$socket->recovered` 属性，支持状态恢复、房间恢复和丢失消息重发；优化项目目录结构
